...
//LCD的画笔颜色和背景色	   
uint16_t POINT_COLOR=0x0000;	//画笔颜色：黑
...
{  
	//SPI1_Init();		   			        //初始化SPI
	LCD18_GPIOInit();//LCD GPIO初始化										 
	LCD_RESET(); //LCD 复位

	//************* ST7735S初始化**********//	
	rt_thread_mdelay(120);                //ms                                                              
	LCD_WR_REG(0x11);     
	rt_thread_mdelay(120);                //ms                                                              
	LCD_WR_REG(0x21); 
	LCD_WR_REG(0xB1);     
...
{	
	...
	LCD_WR_DATA(0x00);
	LCD_WR_DATA(xStart+lcddev.xoffset);		
	LCD_WR_DATA(0x00);
	LCD_WR_DATA(xEnd+lcddev.xoffset);

	LCD_WR_REG(lcddev.setycmd);	
	LCD_WR_DATA(0x00);
	LCD_WR_DATA(yStart+lcddev.yoffset);		
	LCD_WR_DATA(0x00);
